...
from app.api.deps import get_sales_global_filters, get_sales_scoped_filters , get_conn
...
def attach_transaction_links(item: list[dict]) -> None:
    ...
    links = {
        "self": f"/sales-transactions/{uuid}",
    }

    if postcode:
        links["postcode"] = f"/postcodes/{postcode}/sales-transactions"

    if area_code:
        links["area"] = f"/areas/{area_code}/sales-transactions"
...
@router.get(
    "/sales-transactions/{transaction_uuid}",
...
def api_get_official_sales_transaction(
    ...
    item = get_official_sales_transaction_by_uuid(conn, transaction_uuid)
    if not item:
...
@router.get(
    "/areas/{area_code}/sales-transactions",
    response_model=PagedResponse[OfficialSalesTransactionOut],
...
def api_list_official_sales_by_area(
    ...
    result = list_official_sales_transactions_by_area(conn, area_code, filters)
    if result is None:
...
@router.get(
    "/postcodes/{postcode}/sales-transactions",
    response_model=PagedResponse[OfficialSalesTransactionOut],
...
def api_list_official_sales_by_postcode(
    ...
    result = list_official_sales_transactions_by_postcode(conn, postcode, filters)
    if result is None:
        raise HTTPException(status_code=404, detail="Area not found")
